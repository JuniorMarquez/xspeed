	...
	public function index($setting) {
		...
		if ($results) {
			foreach ($results as $result) {
				...
				$results_imgs = $this->model_catalog_product->getProductImages($result['product_id']);

				foreach ($results_imgs as $img) {
					$images[] = $this->model_tool_image->resize($img['image'], 100, 100);
				}
				...
				$data['products'][] = array(
					...
					'description2' => utf8_substr(strip_tags(html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8')), 0, 200) . '..',
					'price'       => $price,
					'special'     => $special,
					'tax'         => $tax,
					'rating'      => $rating,
