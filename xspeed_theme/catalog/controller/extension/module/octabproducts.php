...
class ControllerExtensionModuleOcTabProducts extends Controller {
	public function index($setting) {
		...
		if(isset($setting['module_image'])) {
			$thumb = $this->model_tool_image->resize($setting['module_image'], 461, 436);
		} else {
		...
		if(isset($setting['module_image'])) {
			$thumb2 = $this->model_tool_image->resize($setting['module_image'], 208, 419);
		} else {
			$thumb2 = $this->model_tool_image->resize('no_image.png', 100, 100);
		}
		
		if(isset($setting['module_image'])) {
			$thumb3 = $this->model_tool_image->resize($setting['module_image'], 320, 680);
		} else {
			$thumb4 = $this->model_tool_image->resize('no_image.png', 100, 100);
		}
		
		if(isset($setting['module_image'])) {
			$thumb4 = $this->model_tool_image->resize($setting['module_image'], 799, 277);
		} else {
			$thumb4 = $this->model_tool_image->resize('no_image.png', 100, 100);
		}
		...
		// for($i=0;$i<count($title2);$i++){ 						
			// $headingtitle .= $i>1 ? "<span class='word2'>".$title2[$i]."&nbsp;</span>" : "<span>".$title2[$i]."&nbsp;</span>";
		// }
